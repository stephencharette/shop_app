    ...
    records.map do |record|
      {
        ...
        edit: link_to(
          heroicon('pencil', variant: :outline,
                             options: { class: 'hover:text-indigo-700 hover:cursor-pointer h-4' }), edit_user_path(record.user_id)
        ),
        destroy: link_to(
          heroicon('trash', variant: :outline,
                            options: { class: 'hover:text-indigo-700 hover:cursor-pointer h-4' }), user_path(record.user_id),
          method: :delete
        )
      }
    end
